  ...
  return (
    <>
      ...
      <HeroSlider>
        ...
        <h1 className="mt-2 max-w-3xl font-display text-[1.45rem] leading-[1.18] font-semibold tracking-tight sm:mt-5 sm:text-5xl lg:text-6xl">
          Des solutions durables pour vos{" "}
          <span className="text-accent">projets industriels</span>
        </h1>
        <p className="mt-3 max-w-xl text-[0.8125rem] leading-relaxed text-white/70 sm:mt-6 sm:text-base lg:text-lg">
        ...
        <ul className="mt-6 grid max-w-3xl gap-3 sm:mt-14 sm:grid-cols-3 sm:gap-6">
          {trust.map((item, index) => {
            const Icon = trustIcons[index] ?? IconShield;
            return (
              <li key={item.title} className="flex items-start gap-2.5">
                <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/8 text-accent sm:h-9 sm:w-9">
                  <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                </span>
                <div>
                  <p className="text-[0.8125rem] font-semibold sm:text-sm">{item.title}</p>
                  <p className="mt-0.5 hidden text-xs leading-relaxed text-white/50 sm:mt-1 sm:block">{item.text}</p>
                </div>
              </li>
            );
          })}
      ...
      <section className="bg-white py-10 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          />
          <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-12 sm:gap-6 xl:grid-cols-4">
      ...
      <section className="relative overflow-hidden bg-ink py-10 text-white sm:py-20 lg:py-24">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 font-display text-[1.3rem] font-semibold tracking-tight sm:text-3xl md:text-4xl">
          ...
          <dl className="mt-10 grid grid-cols-2 gap-6 sm:mt-14 sm:gap-8 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label}>
                ...
                <dd className="mt-2 font-display text-xl font-semibold text-accent sm:text-3xl md:text-4xl">{stat.value}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <section className="bg-white py-10 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
            ...
            <Link
              href="/realisations"
              className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-ink sm:min-h-11"
            >
              Voir tous les projets
              <IconArrow className="h-3.5 w-3.5" />
            </Link>
          </div>
          <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-12 sm:gap-6 lg:grid-cols-3">
          ...
          <div className="mt-8 divide-y divide-ink/10 sm:mt-10">
            {faqs.map((item) => (
              <details key={item.q} className="group py-4 sm:py-5">
                <summary className="cursor-pointer list-none font-display text-sm font-semibold text-ink marker:content-none sm:text-lg">
      ...
      <section className="relative overflow-hidden bg-ink py-10 text-white sm:py-20">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(245,180,0,0.14),transparent_45%)]" />
        <div className="relative mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
          <p className="text-[0.65rem] font-semibold tracking-[0.22em] text-accent uppercase sm:text-[0.7rem] sm:tracking-[0.28em]">
            Un projet ?
          </p>
          <h2 className="mt-3 font-display text-[1.3rem] font-semibold sm:text-3xl md:text-4xl">
